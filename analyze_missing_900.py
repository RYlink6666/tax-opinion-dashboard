...
# -*- coding: utf-8 -*-
"""
分析缺失的900条记录 (indices 0-899)
基于现有的llm_analyze.py，实现完整的Zhipu API调用
"""
    ...
    for attempt in range(retry_count):
        try:
            ...
            if attempt < retry_count - 1:
                print(f"   ⚠️  API call failed (attempt {attempt+1}/{retry_count}): {str(e)[:50]}")
                time.sleep(2)  # 等待后重试
            else:
                print(f"   ❌ API call failed after {retry_count} attempts")
                return None
